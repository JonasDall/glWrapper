...
void glWrap::Mesh::DrawInstanced(unsigned int count){
...
void glWrap::Model::DrawInstanced(unsigned int count){
    for (auto& mesh : m_meshes){
...
void glWrap::Window::LoadGLTF(std::map<std::string, ModelData>& container, std::string file){
    ...
    for(int i{}; i < model.meshes.size(); ++i){
        ...
        container.emplace(model.meshes[i].name + "." + std::to_string(postfix), ModelData{});
